...
type NewBackendFunc func(*url.URL, string, crypto.EncryptionCodec) (Backend, error)
...
func New(dbURL *url.URL, appName string, codec crypto.EncryptionCodec) (Backend, error) {
	ctor, ok := validDBs[dbURL.Scheme]
	if !ok {
		return nil, errors.New(errors.NotImplementedCause, "database not supported")
	}

	return ctor(dbURL, appName, codec)
}
